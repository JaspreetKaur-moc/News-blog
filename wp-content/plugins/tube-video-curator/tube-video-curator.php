...
class Tube_Video_Curator {    
  ...
  function create_tube_video_post( $args ){ // Setting and calling wp_insert_post();
      ...
      $import_term_ids = intval( get_option( 'tube_vc_default_import_term') );
        
      $import_term_ids = apply_filters( 'tube_vc_filter_import_term', $import_term_ids, $source_site, $creator_name );
      
      if ( $import_term_ids ):
  
        // make sure we're dealing with an array
        if ( ! is_array($import_term_ids) ):
          
           $import_term_ids = array($import_term_ids);
